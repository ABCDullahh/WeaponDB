...
    </div> --}}
    ...
    <div class="container mt-5">
        ...
        <a href="{{ route('negara.create') }}" class="btn btn-primary mb-3"> Tambah Data</a>
        <div class="card">
            <div class="card-body">
                <table class="table">
                    <thead>
                    ...
                    <tbody>
                        @foreach ($negaras as $no => $hasil)
                            <tr>
                                <th>{{ $no+1 }}</th>
                                <th>{{ $hasil-> nama_negara }}</th>
                                <td>{{ $hasil-> region }}</td>
                                <td>{{ $hasil-> presiden }}</td>
                                <td>
                                    <form action="{{ route('negara.destroy', $hasil->id) }}" method="POST">
                                        @csrf
                                        @method('delete')
                                        <a href=" {{ route('negara.edit', $hasil->id) }}" class="btn btn-success btn-sm"> Edit</a>
                                        <button class="btn btn-danger btn">Perma Delete</button>

                                        <form class = "mt-1 form-inline" method="POST" action="{{ route('negara.soft', $hasil->id) }}">
                                            @csrf
